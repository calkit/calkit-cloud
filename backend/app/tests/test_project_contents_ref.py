...
def test_get_contents_from_repo_at_given_ref(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(app.projects, "expand_dvc_lock_outs", lambda *a, **k: {})
    ...
    assert base64.b64decode(item_latest.content).decode().strip() == "version-two"
    ...
    root_latest = app.projects.get_contents_from_repo(project=project, repo=repo)
    latest_names = {item.name for item in (root_latest.dir_items or [])}
    assert "new-file.txt" in latest_names

    root_v1 = app.projects.get_contents_from_repo(project=project, repo=repo, ref=ref_v1)
